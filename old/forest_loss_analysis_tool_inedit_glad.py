...
def forest_loss_function():
    ...
    if column_name2 == "Indonesia":
        lossyr_tcd = ExtractByMask(glad_alerts, fc_geo) * Raster(tcdmosaic) * Raster(confidence) * Raster(idn_prf)
    else:
...
def carbon_emissions_function():
    ...
    if nodata2 == "1":
        ...
        arcpy.AddMessage(column_name2[:3])
        if column_name2[:3] == "IDN":
            arcpy.AddMessage("extracting while masking out prf")
            lossyr_tcd = lossyr_tcd * Raster(tcdmosaic) * Raster(confidence) * Raster(idn_prf)
        else:
            arcpy.AddMessage("extracting without masking prf")
            lossyr_tcd = lossyr_tcd * Raster(tcdmosaic) * Raster(confidence)
        nodata = arcpy.GetRasterProperties_management(lossyr_tcd, "ALLNODATA")
        nodata2 = nodata.getOutput(0)
        if nodata2 == "1":
            arcpy.AddMessage("passing")
